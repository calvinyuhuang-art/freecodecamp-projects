...
echo "$($PSQL "SELECT name FROM teams inner join games on winner_id = team_id where year = 2014 and round = 'Eighth-Final' UNION ALL SELECT name FROM teams inner join games on opponent_id = team_id where year = 2014 and round = 'Eighth-Final' order by name")"
...
echo "$($PSQL "SELECT year, name FROM games inner join teams on games.winner_id = teams.team_id where round = 'Final' order by year")" | sed 's/ //g'
